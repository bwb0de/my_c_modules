...
int _heap_push_down(int node, heap_fighter_item_t *n, int n_size) {
    ...
    heap_fighter_item_t tmp;

    if (li > maxi) {return node;}

    if (li == maxi && ri > maxi) {
        if ( n[ni].iniciativa < n[li].iniciativa ) {
            tmp = n[ni];
            n[ni] = n[li];
            n[li] = tmp;
            return l_node;
        ...
        if (n[li].iniciativa >= n[ri].iniciativa ) {
            //go left
            if ( n[ni].iniciativa < n[li].iniciativa ) {
                tmp = n[ni];
                n[ni] = n[li];
            ...
            //go right
            if ( n[ni].iniciativa < n[ri].iniciativa ) {
                tmp = n[ni];
                n[ni] = n[ri];
                n[ri] = tmp;
                return r_node;
...
int is_heap(heap_fighter_item_t *numeros, int numeros_size) { 
    ...
    for (int i = 0; i < numeros_size; i++) {
        ...
        if (left_child < numeros_size && right_child > numeros_size) { 
            if (numeros[i].iniciativa <= numeros[left_child - 1].iniciativa) {
                return 0;
            }
        } else if (left_child > numeros_size) { break; }
        if (numeros[i].iniciativa < numeros[left_child -1].iniciativa || numeros[i].iniciativa < numeros[right_child - 1].iniciativa ) {
            return 0;
        }
    }
    return 1;
