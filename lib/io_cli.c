...
input_rec_t input_text_on_receiver(const char *etiqueta, input_rec_t ir) {
  ...
  if (fgets(ir.valor.texto, sizeof(ir.valor.texto), stdin) !=
      NULL) { // Validação
...
bool any_in(int *valores_respostas, size_t n_respostas, int *valores_testes,
            size_t n_valores_teste) {
...
input_rec_t input_selection(char *etiqueta, const char **opcoes,
                            int limite_maximo, input_rec_t ir) {
  bool validado = false;
  uint32_t sigma = 0;
  uint8_t passagens = 0;
  ...
  while (1) {
    ...
    parse_selected(ir.valor.texto, endereco_respostas);

    // Validação
    for (int i = 0; i < n_respostas; i++) {
      if (endereco_respostas[i] > limite_maximo) {
        printf("\033[31;1;1mResposta inválida! Selecione apenas entre as "
               "opções disponíveis...\033[0m\n");
        validado = false;
        passagens++;
        break;
      }
