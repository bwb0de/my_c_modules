...
int _heap_push_down(int node, heap_element_t *n, int n_size) {
...
int is_heap(heap_element_t *numeros, int numeros_size) { 
    ...
    for (int i = 0; i < numeros_size; i++) {
        ...
        if (left_child < numeros_size && right_child > numeros_size) { 
            if (numeros[i].score <= numeros[left_child - 1].score) {
                return 0;
            }
        } else if (left_child > numeros_size) { break; }
        if (numeros[i].score < numeros[left_child -1].score || numeros[i].score < numeros[right_child - 1].score ) {
            return 0;
        }
    }
    return 1;
